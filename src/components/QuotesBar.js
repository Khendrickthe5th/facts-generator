...
function QuotesBar() {
    ...
    const getFact = () => {
        setIsLoading(true);
        setFact(<img className="w-[40px] h-[40px] m-auto" height="100" width="100" src={loader} alt="loading"/>);
        
        axios({
            method: "get",
            url: "https://api.api-ninjas.com/v1/facts",
            headers: { 
                'X-Api-Key': "cdjCi0UWRIucRB1GvS9UeIAfuEit0GDg9PJDxvYB", 
    ...
    return (
        <div className="w-[100%] h-[calc(100vh-70px)] flex justify-center items-center px-[40px] dark:bg-transparent">
            <div className="w-[750px] h-fit z-[2]">
                <p className="sm:text-[40px] font-font1 text-[30px] font-bold dark:text-white select-none">
                    {fact}
                </p>
                ...
                {/* Button */}
                <div 
                    onClick={!isLoading ? getFact : undefined} 
                    className="w-[100%] h-fit mt-[50px] mx-auto flex justify-center items-center active:scale-[1.03] cursor-pointer"
                >
                    <div className="w-[150px] h-[40px] sm:w-[250px] sm:h-[70px] relative">
                        <div className="dark:bg-gradient-to-r from-[#6110DB] from-20% via-[#2F89BC] via-45% to-[#00FFA0] to-80% w-[100%] h-[100%] sm:text-[25px] bg-[#009688] text-white font-font1 font-bold flex justify-center items-center select-none">
                            {isLoading ? "Loading..." : "See New Factt"}
                        </div>
